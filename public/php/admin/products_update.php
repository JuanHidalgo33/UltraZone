...
if ($id<=0 || $name==='') { http_response_code(400); echo json_encode(['error'=>'missing_fields']); exit; }

$sql = "UPDATE productos SET name=?, price_cop=?, image_url=?, category=? WHERE productID=?";
$stmt = sqlsrv_prepare($conn, $sql, [$name, $price, $image, $category, $id]);
